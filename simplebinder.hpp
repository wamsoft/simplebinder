...
//--------------------------------------------------------------
// exception handle trapper

template <class TRAP, typename FUNC>
struct ExceptionTrapper {
	typedef typename TRAP::Exception Exception;
	typedef typename TRAP::Message Message;
	typedef typename TRAP::FuncName FuncName;

	ExceptionTrapper(const FUNC &func, const FuncName &name) : func(func), funcname(name) {}
	virtual ~ ExceptionTrapper() {}

	tjs_error invoke(MethodInvokerBase::Vp result, MethodInvokerBase::Num numparams, MethodInvokerBase::Vp *param, MethodInvokerBase::Dp objthis) const {
		Message message;
		try {
			return MethodInvoker<FUNC>::Invoke(func, result, numparams, param, objthis);
		} catch (const Exception &exception) {
			if (!TRAP::Catch(exception, message)) throw;
		}
		return TRAP::Trap(funcname, message);
	}
	template <class FACTORY>
	tjs_error invoke(FACTORY* &result, MethodInvokerBase::Num numparams, MethodInvokerBase::Vp *param, MethodInvokerBase::Dp objthis) const {
		Message message;
		try {
			return FactoryInvoker<FUNC>::Invoke(func, result, numparams, param, objthis);
		} catch (const Exception &exception) {
			if (!TRAP::Catch(exception, message)) throw;
		}
		return TRAP::Trap(funcname, message);
...
struct      ExceptionRethrowTrap {
	typedef ExceptionRethrowTrap<EXCEPTION> SelfType;
	typedef EXCEPTION Exception;
	typedef ttstr* Message;
	typedef const tjs_char *FuncName;

	static bool Catch(const Exception &exception, Message &message) {
		static ttstr error; // [XXX] temporary textwork
		error = ttstr(exception.what());
		message = &error;
		return true; // true:call Trap,  false:throw
	}
	static tjs_error Trap(const FuncName name, const Message message) {
		if (name) TVPThrowExceptionMessage(message->c_str(), name);
		else      TVPThrowExceptionMessage(message->c_str());
		return TJS_E_FAIL;
	}

	template <typename FUNC>
	static ExceptionTrapper<SelfType, FUNC> Get(const FUNC &func, const FuncName name = 0) {
		return ExceptionTrapper<SelfType, FUNC>(func, name);
	}
};

template <class TARGET, int HAS_STATIC>
struct PassThroughInvokerRefStatic { enum { Static = 0 }; };
template <class TARGET>
struct PassThroughInvokerRefStatic<TARGET, 1> { enum { Static = TARGET::Static }; };

template <class PASS, class TARGET, int HAS_STATIC>
struct PassThroughInvoker : public MethodInvokerBase {
	enum { Count = TARGET::Count, Static = PassThroughInvokerRefStatic<TARGET, HAS_STATIC>::Static };
	static inline tjs_error Invoke(const PASS &pass, Vp result, Num numparams, Vp *param, Dp objthis) {
		return pass.invoke(result, numparams, param, objthis);
	}
	template <class FACTORY>
	static inline tjs_error Invoke(const PASS &pass, FACTORY* &result, Num numparams, Vp *param, iTJSDispatch2 *objthis) {
		return pass.invoke(result, numparams, param, objthis);
	}
};
template <class TRAP, typename FUNC>
struct MethodInvoker<ExceptionTrapper<TRAP, FUNC> > : public PassThroughInvoker<ExceptionTrapper<TRAP, FUNC>, MethodInvoker<FUNC>, 1 > {};

template <class TRAP, typename FUNC>
struct FactoryInvoker<ExceptionTrapper<TRAP, FUNC> > : public PassThroughInvoker<ExceptionTrapper<TRAP, FUNC>, FactoryInvoker<FUNC>, 0 > {};
...
using Detail::ExceptionRethrowTrap;
